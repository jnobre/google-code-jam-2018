...
int main(){
	...
	for(int ct = 1 ; ct <= t ; ct++) {
		...
		while(i < s){
			...
			if(resFwd.first > best) {
				best = resFwd.first;
				bestCnt = 1;
			} else if(resFwd.first == best)
				bestCnt++;

			auto resBck = max(search(i, da[i], INF, -1), search(i, INF, db[i], -1));
			if(resBck.first > best) {
				best = resBck.first;
				bestCnt = 1;
			} else if(resBck.first == best && resBck.second != resFwd.second)
				bestCnt++;
